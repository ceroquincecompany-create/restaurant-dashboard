...
import {
  ...
  Pencil, BookCheck, Clock, CloudUpload,
} from 'lucide-react'
...
type ItemCola = {
  uid: string
  file: File
  status: 'pendiente' | 'procesando' | 'guardado' | 'error'
  error?: string
}
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs font-semibold text-gray-600">
            {procesando ? 'Procesando...' : 'Cola de subida'} — {guardados}/{total} completados
        ...
        <div className="w-24 h-1.5 bg-gray-200 rounded-full overflow-hidden">
          <div
            className="h-full bg-[#F5B731] rounded-full transition-all"
        ...
        {cola.map(item => (
          <li key={item.uid} className="flex items-center gap-3 px-4 py-2.5">
            <div className="flex-shrink-0">
              {item.status === 'guardado'   && <CheckCircle2 size={15} className="text-emerald-500" />}
...
export default function PaginaFacturas() {
  ...
  async function lookupOrCreateProveedor(nombre: string | null, cif: string | null): Promise<number | null> {
    ...
    if (nombre) {
      ...
      const { data: nuevo } = await supabase
        .from('proveedores')
        .insert({ nombre: nombre.trim(), cif: cif ?? null })
        .select('id')
        .single()
  ...
  async function procesarItem(item: ItemCola) {
    ...
    try {
      ...
      // 3. Supabase Storage (opcional, no bloquea si falla)
      ...
      try {
        ...
        if (stored) {
          const { data: { publicUrl } } = supabase.storage
            .from('facturas-proveedores').getPublicUrl(path)
          archivo_url = publicUrl
        }
      } catch { /* bucket no configurado todavía */ }

      // 4. Proveedor
      const proveedor_id = await lookupOrCreateProveedor(datos.supplier_name, datos.supplier_cif)

      // 5. Insertar factura
      const fecha = datos.date ?? null
      const { data: factura, error: fErr } = await supabase
        ...
        .insert({
          ...
          metodo_extraccion: datos._metodo ?? 'haiku',
          estado:            'pendiente',
        })
        .select('id')
        .single()
  ...
  async function contabilizar() {
    ...
    const { data: existing } = await supabase
      .from('pl_datos').select('id, valor_real')
    ...
    if (existing) {
      ...
      await supabase.from('pl_datos').insert({ local_id: lid, año: anio, mes, partida: 'proveedores', valor_real: nuevoValor, valor_presupuesto: 0 })
    }
    const now = new Date().toISOString()
    await supabase.from('facturas').update({
      estado: 'contabilizada', contabilizado: true, pl_mes: mes, pl_anio: anio, pl_contabilizado_at: now,
    }).eq('id', modalCont.id)
    setFacturas(prev => prev.map(f => f.id === modalCont.id
      ? { ...f, estado: 'contabilizada', contabilizado: true, pl_mes: mes, pl_anio: anio, pl_contabilizado_at: now }
      : f
    ))
      ...
      {mostrarSubida && (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <select
              value={localSubida ?? ''}
              onChange={e => setLocalSubida(Number(e.target.value))}
        ...
        <Modal titulo={`Factura — ${detalle.proveedor_nombre ?? '—'}`} onCerrar={() => setDetalle(null)} maxW="max-w-3xl">
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-2">
              ...
              {detalle.contabilizado && detalle.pl_mes && (
                <span className="flex items-center gap-1 text-xs text-emerald-700 font-semibold">
                  <BookCheck size={12} />
                  Contabilizado en P&L — {MESES[detalle.pl_mes - 1]} {detalle.pl_anio}
                </span>
              )}
            </div>
            ...
            {detalle.facturas_lineas.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Líneas de factura</p>
              ...
              <div className="px-5 py-4 space-y-4">
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
                  ...
                  <p className="text-xs text-emerald-700 mt-0.5">
                    ...
                    {modalCont.base_imponible != null
                      ? ' — usando base imponible (sin IVA)'
                      : ' — usando total factura'}
                  </p>
                </div>
