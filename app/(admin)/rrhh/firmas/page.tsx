...
import {
  Plus, X, RefreshCw, FileSignature, CheckCircle, Clock,
...
const PLANTILLAS: Record<string, { tipo: string; titulo: string; texto: string }> = {
  vacaciones: {
    tipo: 'Vacaciones aceptadas',
    titulo: 'Aceptación de período vacacional',
    texto: `Yo, el/la empleado/a abajo firmante, declaro haber recibido la comunicación relativa al período vacacional asignado y doy mi conformidad expresa con las fechas indicadas en el calendario laboral acordado.
...
Confirmo que he sido informado/a con la antelación legalmente establecida y acepto voluntariamente el período de vacaciones asignado, sin perjuicio de los derechos que me correspondan según el Convenio Colectivo aplicable.`,
  ...
  rgpd: {
    tipo: 'RGPD-LOPD',
    titulo: 'Consentimiento de Protección de Datos (RGPD/LOPDGDD)',
    texto: `De conformidad con el Reglamento (UE) 2016/679 General de Protección de Datos (RGPD) y la Ley Orgánica 3/2018 de Protección de Datos Personales y garantía de los derechos digitales (LOPDGDD), presto mi consentimiento expreso para que la empresa trate mis datos personales con las siguientes finalidades:
...
Los datos no serán cedidos a terceros salvo obligación legal. Puedo ejercer mis derechos de acceso, rectificación, supresión, limitación, portabilidad y oposición dirigiéndome a la empresa por escrito.`,
  },
  confidencialidad: {
...
El incumplimiento de este acuerdo facultará a la empresa para ejercitar las acciones legales y reclamar los daños y perjuicios que correspondan.`,
  },
}
...
export default function PaginaFirmasAdmin() {
  ...
  const cargar = useCallback(async () => {
    ...
    setDocs((d ?? []).map((x: DocumentoFirma) => ({
      ...x,
      firmas: firmasData.filter(f => f.documento_id === x.id),
    })))
    setEmpleados(e ?? [])
    setLoading(false)
  }, [])
      ...
      {docs.length === 0 ? (
        ...
        <div className="space-y-2">
          {docs.map(d => {
            ...
            const pendientes = d.firmas.filter(f => !f.firmado)
            ...
            return (
              <div key={d.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div
                  className="px-5 py-4 flex items-center gap-4 cursor-pointer hover:bg-gray-50 transition-colors select-none"
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-xs text-gray-400">
                        {d.empleado_id === null ? 'Todos los empleados' : (emp?.nombre ?? '—')}
                      </span>
                      ...
                      {d.fecha_limite && (
                        <>
                          <span className="text-gray-200">·</span>
                          <span className="text-xs text-gray-400 flex items-center gap-1"><Calendar size={10} /> Límite: {fmtFecha(d.fecha_limite)}</span>
                        </>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {confirmEliminar === d.id ? (
                      ...
                      <button onClick={e => { e.stopPropagation(); setConfirmEliminar(d.id) }} className="p-1.5 text-gray-300 hover:text-rose-500 transition-colors rounded">
                        <Trash2 size={14} />
                      </button>
                    )}
                      ...
                      {faltanFirmar.length > 0 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {faltanFirmar.map(e => (
                              <span key={e.id} className="text-xs bg-amber-50 border border-amber-200 text-amber-700 px-2 py-0.5 rounded-full">{e.nombre}</span>
                            ))}
                          </div>
                        </div>
                      )}

                      {d.empleado_id !== null && pendientes.length === 0 && firmadas.length === 0 && (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {Object.entries(PLANTILLAS).map(([key, p]) => (
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-semibold bg-gray-100 text-gray-700 hover:bg-[#F5B731] hover:text-[#1A1A1A] rounded-lg transition-colors"
                    >
                      {p.tipo}
                    </button>
                  ))}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Texto completo del documento que el empleado leerá antes de firmar..."
                />
              </div>
