...
const AVATAR_COLORS = ['bg-rose-400','bg-blue-400','bg-emerald-400','bg-violet-400','bg-amber-400','bg-cyan-400']
...
export default function ChatEmpleado() {
  ...
  useEffect(() => {
    cargar()
    const ch = supabase.channel('chat_empleado')
    ...
    <div className="flex flex-col" style={{ height: 'calc(100vh - 128px)' }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-200 flex-shrink-0">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-bold text-gray-900">Chat de equipo</p>
            <p className="text-xs text-gray-400">Canal general</p>
          </div>
        </div>
      </div>
          ...
          msgs.map(m => {
            ...
            return (
              <div key={m.id} className={`flex items-end gap-2 ${esPropio ? 'flex-row-reverse' : ''}`}>
                <div className={`w-7 h-7 rounded-full ${avatarColor(m.empleado_nombre)} flex items-center justify-center text-white text-xs font-bold flex-shrink-0`}>
                  {m.empleado_nombre.charAt(0).toUpperCase()}
                </div>
                <div className={`max-w-[72%] flex flex-col ${esPropio ? 'items-end' : 'items-start'}`}>
                  {!esPropio && <p className="text-[11px] text-gray-500 mb-1 ml-0.5">{m.empleado_nombre}</p>}
                  ...
                  <p className="text-[10px] text-gray-400 mt-0.5 mx-0.5">{fmtHora(m.created_at)}</p>
                </div>
              </div>
            )
          })
      ...
      {/* Input */}
      <form onSubmit={enviar} className="flex items-end gap-2 px-4 py-3 bg-white border-t border-gray-200 flex-shrink-0">
        <textarea
          value={texto}
          ...
          rows={1}
          className="flex-1 px-4 py-2.5 text-base border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#F5B731] resize-none"
        />
        <button
          type="submit"
          disabled={!texto.trim() || enviando}
          className="w-10 h-10 bg-[#F5B731] text-[#1A1A1A] rounded-xl flex items-center justify-center disabled:opacity-40 flex-shrink-0 active:scale-95 transition-all"
        >
          <Send size={16} />
        </button>
      </form>
