...
function TabMermas({ empleado, localId, today }: { empleado: any; localId: number | null; today: string }) {
  ...
  const ingFiltrados = useMemo(() => ingredientes.filter(i => i.nombre_ingrediente.toLowerCase().includes(busq.toLowerCase())).slice(0, 8), [ingredientes, busq])
  ...
  function seleccionar(ing: Ingrediente) {
    ...
    if (cant > 0 && ing.precio_unidad_producto) setCantidad(cantidad)
  }
  ...
  async function guardar() {
    ...
    await supabase.from('mermas').insert({
      local_id: localId,
      empleado_nombre: empleado?.nombre ?? '',
      tipo, ingrediente_id: Number(selId),
      cantidad: Number(cantidad), coste,
      fecha: today,
      notas: notas.trim() || null,
    })
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        {ingSel && <p className="text-sm text-gray-400 mt-1">Unidad: <strong>{ingSel.unidad_producto}</strong>{ingSel.precio_unidad_producto != null && ` · ${ingSel.precio_unidad_producto} €/ud`}</p>}
      </div>
